...
export function useChat(sessionId: string, initialMessages: Message[]) {
  ...
  const runStream = useCallback(
    async ({ request, mode, model }: RunStreamParams) => {
    ...
    [updateMessages, isActiveRequest, clearStream, handleStream],
  );
  ...
  const submit = useCallback(
    ...
    [sessionId, runStream, updateMessages, stopActiveStream],
  );
  ...
  return { messages, streaming, submit, abort, interrupt, resume };
}
